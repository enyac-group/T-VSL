...
from models.audioclip.esresnet import attention
...
class ResNetWithAttention(it.AbstractNet):
    ...
    def _forward_reduction(self, x: torch.Tensor, return_feats = False) -> torch.Tensor:
        if self.apply_attention:
            if return_feats:
                feat_attn = x.clone()
                feat_attn = self.att5(feat_attn, x.shape[-2:])
                feat = x * feat_attn            
    ...
    def _forward_classifier(self, x: torch.Tensor) -> torch.Tensor:
        if x.ndim == 4:
            ...
            x = x.permute(0, 2, 3, 1)
            x = x.contiguous().view(batch, -1, dim)
    ...
    def forward(self,
                x: torch.Tensor,
                return_feats: torch.bool = False,
                y: Optional[torch.Tensor] = None) -> Union[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:

        x = self._forward_pre_processing(x)
        x = self._forward_features(x)
        if return_feats:
            x, feat = self._forward_reduction(x, return_feats=return_feats)
            feat = self._forward_classifier(feat)
        else:
        ...
        if return_feats:
            return (y_pred, feat) if loss is None else (y_pred, feat, loss)
...
class _ESResNet(ResNetWithAttention):
    ...
    def _forward_reduction(self, x: List[torch.Tensor], return_feats: torch.bool = False) -> torch.Tensor:
        if return_feats:
            outputs = list()
            feats = list()
            for ch in x:
                out, feat = super(_ESResNet, self)._forward_reduction(ch, return_feats)
                outputs.append(out)
                feats.append(feat)
        ...
        outputs = list()
        for ch in x:
            out = super(_ESResNet, self)._forward_reduction(ch, return_feats)
            outputs.append(out)
        outputs = torch.stack(outputs, dim=-1).sum(dim=-1)
